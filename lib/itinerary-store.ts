...
async function saveItineraryToAPI(
  ...
  try {
    const {
      createItinerary,
      updateItinerary,
      getItinerary: getItineraryFromAPI,
    } = await import('./api/itineraries')
    ...
    if (!user) {
      throw new Error(
        'User must be authenticated to save itineraries'
      )
    }
...
async function updateItineraryInAPI(
  itin: Itinerary
): Promise<Itinerary> {
  try {
    ...
    console.warn(
      'Failed to update in Supabase, falling back to localStorage:',
      error
    )
    // Fallback to localStorage on API failure
    setItinerary(itin)
...
export async function confirmBooking(): Promise<Itinerary | null> {
  ...
  try {
    ...
    const { getItinerary: getItineraryFromAPI } = await import(
      './api/itineraries'
    )
    ...
    try {
      ...
      const existingItinerary = await getItineraryFromAPI(
        currentItinerary.id
      )
      if (existingItinerary) {
...
export function appendSpot(
  ...
  const list = days[dayIndex]
    ? [...days[dayIndex], newSpot]
    : [newSpot]
  const next: Itinerary = {
    ...current,
    days: { ...days, [dayIndex]: list },
  }
  setItinerary(next)
