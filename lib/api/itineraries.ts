...
export async function getItineraries(
  ...
  try {
    if (!userId) {
      ...
      if (!user) {
        console.warn(
          'No user authenticated, returning empty itineraries'
        )
        return []
      }
      userId = user.id
...
export async function createItinerary(
  ...
  if (!userId) {
    ...
    if (!user)
      throw new Error(
        'User must be authenticated to create an itinerary'
      )
    userId = user.id
  }
...
export async function updateItinerary(
  ...
  const supabaseItinerary = await itinerariesApi.update(
    id,
    supabaseData
  )
  return convertFromSupabase(supabaseItinerary)
