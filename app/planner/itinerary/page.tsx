...
export default function ItineraryPage() {
  ...
  const fromParam =
    itinerary?.start || search.get('from') || undefined
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'ArrowLeft')
        setActiveDay((d) => clamp(d - 1, 1, totalDays))
      if (e.key === 'ArrowRight')
        setActiveDay((d) => clamp(d + 1, 1, totalDays))
    }
    ...
    <div className="mx-auto max-w-[720px] px-4 py-8">
      ...
      <div className="mt-8 space-y-4">
        ...
        <button
          ...
          className={`w-full rounded-full py-4 text-lg font-semibold text-white shadow-md transition duration-300 ${
            allDaysHaveActivities && !isUpdating
